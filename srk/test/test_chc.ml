...
let countup1 () =
  ...
  let () =
    let open Infix in
    add_rule fp [atom1] (x' = x + (int 1)) atom2; 
    add_rule fp [atom2] (x = x' + (int 1)) atom1;
    add_rule fp [] ((int 0) <= x) atom1;
    add_rule fp [atom2] (x' <= (int 0)) error_atom;
    add_query fp error;
  in
  let res = check srk fp pd in
  assert (res = `No)
...
let countup2 () =
  ...
  let () =
    ...
    add_rule fp [atom2] (x = x' + (int 1)) atom1; 
    add_rule fp [] ((int 0) <= x) atom1;
    add_rule fp [atom2] ((int 0) <= x') error_atom;
    add_query fp error;
  in
  let res = check srk fp pd in
  assert (res = `Unknown)
...
let xskipcount () =
  ...
  let () =
    let open Infix in
    add_rule fp [] (y' = x') (postify vert.(0));
    add_rule fp [vert.(0)] (x' = x + (int 1) && y' = y) (postify vert.(1));
    add_rule fp [vert.(0)] (x' = x && y' = y) (postify vert.(1));
    add_rule fp [vert.(1)] (y' = y + (int 1) && x' = x) (postify vert.(0));
    add_rule fp [vert.(0)] (y' < x' && x' = x && y' = y) (postify vert.(2));
    add_query fp (rel_of_atom vert.(2));
  in
  let res = solve srk fp pd in
...
let dupuncontrsym () =
  ...
  let () =
    let open Infix in
    add_rule fp [] (y = (int 1)) vert.(0);
    add_rule fp [vert.(0)] (y = (int 0)) vert.(1);
  in
  let res = solve srk fp pd in
